...
// The SSR sample is verified in Node because the runtime report is produced by compiling and executing
// a dedicated server workspace. Browser-style component rendering would miss the actual stream behavior.
...
describe("react streaming SSR workspace", () => {
  it("compiles and executes the runtime report", () => {
    ...
    if (!artifact) {
      throw new Error(
        "Missing artifact config for sample-react-streaming-ssr.",
      );
    }
    ...
    execFileSync(
      ...
      [
        ...
        "-p",
        "server-samples/react-streaming-ssr/tsconfig.runtime.json",
      ],
      {
        cwd: process.cwd(),
        stdio: "pipe",
    ...
    const output = execFileSync(
      process.execPath,
      ["server-samples/react-streaming-ssr/dist/runAllModes.js"],
      {
        cwd: process.cwd(),
        encoding: "utf8",
        stdio: "pipe",
    ...
    // Each key in this report corresponds to a different server-rendering API. The tests stay explicit
    // instead of looping generically because each mode promises different fallback, boundary, and resume behavior.
    ...
    expect(report.resumeToPipeableStream.resumedContainsResolvedContent).toBe(
      true,
    );
    ...
    expect(report.resumeAndPrerenderToNodeStream.containsResolvedContent).toBe(
      true,
    );
  });
});
